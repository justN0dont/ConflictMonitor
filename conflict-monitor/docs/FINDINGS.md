...
- [ ] `feed_health` table written from each poller's existing loop; `/health` exposing it — **aircraft, AIS and satellite slices shipped 2026-09-24**: the in-memory tracker, the envelope on all three tracking routes, and `GET /health` reading the trackers. Not yet: connectivity (which already has its own honest vocabulary, pinned by `test_connectivity.py`) and the RSS lane, and persistence (the transition and heartbeat rows of decision 3), which is what ticks this box
