...
60 candidate findings, first re-checked against the tree at `64b690a` and re-verified line by line against `de146f6`: **25 open**, 32 fixed, 3 invalid or external. Of the open ones, **none is critical** and 8 are high. (Recounted from the rows by script on 2026-09-26, when `C69` was rebased onto `0f6e1ea`: the two branches closed `C69` and `C60`/`C61`/`C62` in parallel, each recounted correctly against a base without the other's move, and the merge left both sentences one side short. Earlier recount, kept: on 2026-09-25, when `C60`, `C61` and `C62` moved to Fixed: 29 open and 11 high before, and the tables agreed. Earlier recount, kept: on 2026-09-24, when `C31` moved to Fixed: before the move the tables already held 30 open and 12 high against the 31 and 13 this line claimed, so it had drifted by one again. Earlier note, kept: `8523961` closes `C10`, narrows `C11` and `C74`, and opens `C78` and `C79`. It also corrects the line itself: the previous text said "57 / 29 open / 14 high" while the tables held 58 rows and 30 open — the note claimed `ce5d994` opened `C76` when it opened `C76` **and** `C77`, and the tally was incremented once. The same drift, in the same sentence, two commits running. Counted from the rows with a script, not carried forward.)
...
| `C66` | medium | — | Platform | **Narrowed in *this commit*.** 103 tests now cover the behaviours the last two days verified by hand, and each one was demonstrated to FAIL against its own defect re-introduced (nine mutations, nine catches). **CI half closed 2026-09-24:** `.github/workflows/ci.yml` runs the suite (176 tests at 2026-09-24, one of them the C31 strict xfail) against a PostGIS service container and builds the frontend on every push. What stays open: the suite cannot see schema drift, because the test database comes from `create_all` while production's comes from the hand-kept ALTER list (`C63`). The counter-test is named in `tests/conftest.py` and deliberately not written yet |
...
| `C71` | medium | 3 | Platform | Demo rows ARE tagged source='demo' in the DB and API; it is the UI that discards the distinction. **Narrowed by `C69`'s fix (*this commit*):** every demo row generated from that commit on starts its summary and raw_text with `SYNTHETIC · `, so the distinction reaches every surface that prints the text. Still open: the UI does not read `source`, and an instance in either mode serves any demo rows already in its table, including rows written before that commit, which carry no marker (`GODS-EYE-VIEW.md` SAFETY-10 item 4) |
...
| ID | Sev | Phase | Area | Finding |
|---|---|---|---|---|
| `C60` | high | — | Platform | CORS is an allow-list from `CORS_ORIGINS` (default the Vite dev origin on localhost and 127.0.0.1), `GET` only, no credentials: the finding's own preflight from `https://attacker.example` now answers 400 with no `allow-origin`. The websocket refuses a browser `Origin` outside the same list with close code 1008 (CORS never covered it). All three ports are published on `127.0.0.1` only. What this does **not** do: the read API and the live feed still have no authentication, so anyone who can reach port 8000 can read them — a choice for a single-operator install on loopback, stated in the README's Security section, and not a property that survives re-publishing the port (`0f6e1ea`) |
| `C62` | high | — | Platform | Every `/events/admin/*` route is declared on one `admin_router` whose dependency checks `X-Admin-Token` against `ADMIN_TOKEN` in constant time; with no token set the routes answer 503, so an unset secret closes them rather than opening them. A test walks the app's routes and fails on any admin path or any non-GET route outside that dependency, so a later bare `@router.delete` fails in CI, not in production. Checked live: 401 without or with a wrong token, 200 with it (`0f6e1ea`) |
| `C61` | medium | — | Platform | Postgres is published on `127.0.0.1:5432`, and the backend's `DATABASE_URL` is built from `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB`, so changing the password changes both sides (`docker compose config` shows it). The healthcheck uses the same user variable. The frontend container no longer takes `env_file: .env` — it held every backend secret — and gets only its `VITE_*` values, which are public by design (`0f6e1ea`) |
| `C31` | high | 2 | Collection | Aircraft half fixed (*this commit*): every poll attempt is recorded by a feed tracker (`app/feeds.py`), `/tracking/aircraft` returns an envelope whose `count` is null unless the feed is live or stale, and `/tracking/jamming` withholds its last verdict as `feed_not_live` once the feed stops. Reproduced dead live, before and after. Vessels and TLEs still serve bare arrays (untriaged: bare-array tracking routes) |
...
| `C69` | high | — | Platform | Demo bylines are fictional (`DEMO-CH-01` to `DEMO-CH-11`) and every demo row generated from *this commit* on starts its summary and raw_text with `SYNTHETIC · `, so one row cut out of a screenshot still says what it is. The nine templates that put confirmations, statements, threats, requests and offers in real actors' mouths now report them in the passive (*this commit*). `tests/unit/test_demo.py` fails if a byline is not in the `DEMO-CH-NN` form, matches a `CHANNEL_REGISTRY` key or display name, or a generated row lacks the marker. Residual, not in this fix: demo rows already stored keep their real bylines and old wording, with no marker, until cleared (`DELETE FROM events WHERE source = 'demo'`; their report rows go by cascade); demo rows still write no `event_reports` rows; and nothing hides demo rows from a live-mode instance (`C71`; SAFETY-10 items 3 and 4) |
...
**Now a test, not only prose (2026-09-24).** `tests/unit/test_aircraft_poll.py::test_after_both_sources_fail_the_verdict_is_not_still_ok` drives the real `start_opensky_poller` loop with both fetchers scripted (a success, then a double failure) and asserts the jamming status no longer reads "ok". It is `xfail(strict=True, raises=AssertionError)`: it fails today for the stated reason, and the day feed_health fixes C31 it XPASSes, which `xfail_strict` turns into a failure, so the marker has to come off in the fixing commit. Checked both ways: it XFAILs against the current tree, and a simulated fix (writing a non-"ok" status on the failure branch) made the run fail with `[XPASS(strict)]`. A companion test pins the half that is already right: `as_of` stays at the last success.
...
**Fixed 2026-09-25** (see the Fixed table): an origin allow-list from `CORS_ORIGINS`, `GET` only, no credentials; an `Origin` check on the websocket; ports on loopback. The read API itself stays unauthenticated by choice, which the Fixed row states.
...
**Fixed 2026-09-25**, as proposed, with one change: an empty `ADMIN_TOKEN` does not refuse startup, it makes the admin routes answer 503. Closing the routes gives the same guarantee without making the whole monitor fail to boot over a maintenance feature.
...
- ~~**No CI.**~~ **Closed 2026-09-24.** The repo has a remote now, and `.github/workflows/ci.yml` runs on every push and pull request: `pytest -q` on Python 3.12 against `postgis/postgis:16-3.4` (the image `docker-compose.yml` uses) with `LLM_BACKEND=none`, and `npm ci && npm run build` on Node 20, whose `tsc -b` fails the job on a type error. Rehearsed before it was pushed, with the job's own commands on a fresh virtualenv and a fresh PostGIS cluster: 154 passed in 6.55 s. It was also shown to fail twice. With no reachable database all 154 tests **error**, which is the design in `pytest.ini`, not a skip. With `"think": False` flipped back in `classifier.py`, `test_the_payload_turns_thinking_off` fails and the job exits 1. Two requirements were learned from running it rather than read from the README: `tests/unit` needs Postgres too, because the session fixture builds the test database for every test, and the role must be a superuser, because the conftest runs `DROP DATABASE … WITH (FORCE)` and `CREATE EXTENSION postgis`.
...
**Fixed 2026-09-25 (*this commit*), and more than the one line above.** The fictional bylines alone do not survive a screenshot cropped to the summary, so every demo row generated from here on also starts its summary and raw_text with `SYNTHETIC · `, set once in `_gen_event`, the builder all three demo writers use. It is set at generation and repairs nothing already stored: a database that ran demo mode before this commit still serves those rows, with real bylines and no marker, until they are cleared with `DELETE FROM events WHERE source = 'demo'`. Deleting them at startup was left out on purpose, because SAFETY-10 prescribes hiding demo rows rather than deleting them and a boot-time delete is a decision, not a repair. Nine templates that put words in real actors' mouths ("IDF confirms", "CENTCOM confirms", "Iran threatens", "Russia calls for", "China offers" and four more) are now reported in the passive; templates naming a real actor's *action* at a real place stay, because the marker now travels with them. Measured rather than recalled: 9 of the 11 old bylines match a `CHANNEL_REGISTRY` key or display name once case and punctuation are ignored, six exactly and three by one name containing the other (`CIG` in `CIG_telegram`, `IranIntl` in `IranIntl_En`, `WarMonitor` in `War Monitors`), which is the rule the test applies. The other two, `MidEast Spectator` and `QudsAlert`, match nothing there and are still real outlets' names, which is why the test requires the `DEMO-CH-NN` form and does not settle for a registry miss. Shown to fail before it was believed: the old bylines, one real name disguised as `DEMO-CH-WarMonitors`, the marker removed, and the marker kept on summary but dropped from raw_text, re-introduced one at a time; four mutations, four catches. `evidence_span` is unchanged on demo rows: the writers pass an empty `location_name`, for which it returns `''` before reading the text.
...
- MEDIUM, **observed 2026-09-25, from the cloud container, not from the production host** — four of the nine RSS feeds deliver nothing, which the `rss` tracker's `parts` showed on its first cycle and which was invisible before (a non-200 returned silently). `reuters` (`feeds.reuters.com`): no answer at all — that host was retired years ago. `ap_news`: HTTP 403 with a Cloudflare "Just a moment..." challenge page. `times_of_israel`: HTTP 403. `iran_international` (`/en/rss`): HTTP 200 with an HTML page, not a feed, so the URL is wrong rather than blocked. The two 403s may be datacenter-IP blocks that a home connection would not hit; re-check from the production host before replacing anything. Not fixed here: which outlets replace them is an editorial decision (the tiering in `news_feeds.py`'s docstring), not a mechanical one. The corroboration score has in practice been drawing on five outlets, not nine.
...
- /tracking/aircraft, /tracking/vessels and /tracking/tle all return bare arrays with no as_of and no coverage (routes/tracking.py:11-39); maritime.py:134 maintains `_cache["last_update"]` that nothing ever reads. Header.tsx:127 renders "0 SAT" for a dead CelesTrak feed exactly as it would for an empty sky, and "0 VES" likewise - the status…
  **Aircraft half fixed 2026-09-24:** `/tracking/aircraft` now returns the feed envelope (FINDINGS decision 4); **vessels and TLEs too, the same day:** a missing AIS key reads `unconfigured` and "— VES", no longer "0 VES"; a dead CelesTrak reads "— SAT" with a reason. **All three tracking routes now return the envelope; this item is closed.**
...
- maritime.py:102-104 returns from _run_websocket on any server `error` frame and start_maritime_poller (:56-64) reconnects every 10s forever with no backoff and no surfaced state, so an invalid API key or a rejected bounding box becomes a silent 6-attempts-per-minute loop visible only in the log.
  **Closed 2026-09-24.** Reconnects climb a ladder (5, 15, 60, 300 s, then every 900 s; under 10 attempts an hour against the old 360), a key error frame waits an hour, every read has a 300 s deadline so an open-but-silent socket is recycled, only transitions are logged, and every attempt reaches the feed tracker. **Measured while closing it, and it changes one assumption:** a bogus key against the real AISStream did not produce the key error frame this code classifies as `auth_failed`. The server accepted the connection and the subscription, then dropped the socket with no close frame (`ConnectionClosedError: no close frame received or sent`), four times in ~100 s on the ladder. So a bad key currently reads as `fetch_error` → `pending` → `unavailable`, not `auth_failed`. Nothing here guesses at telling the two apart; a real key is needed to see what a healthy session and a revoked one look like.
- TLE epoch age is never checked or displayed: CesiumView.tsx:611-620 and GlobeView.tsx:341-347 feed whatever line1/line2 arrive straight into twoline2satrec/propagate. Once C20 is fixed with on-disk persistence, a month-old element set will propagate silently with kilometres of drift and no currency indicator - the satellite layer's versio…
  **Closed 2026-09-24.** The epoch is parsed from line 1 (two-digit year pivoted at 57) and both lines are checksum-verified; malformed records are rejected and counted, never dated by guess. The satellites feed's upstream clock is the newest epoch, so a fetch of orbits older than 14 days reads `stale` (a choice recorded in `app/feeds.py`, not a measurement). The rail's new SATELLITES row prints the newest element set's age beside the fetch age, computed on the server's clock. A disk-cache load carries the time the file was fetched, not the time it was loaded. Not done: dropping records whose `twoline2satrec` fails in the two views (GODS-EYE-VIEW.md COLLECTION-12), and per-group outcomes, which wait for a second `TLE_GROUPS` entry (a comment at the success test says so).
...
- [x] `feed_health` table written from each poller's existing loop; `/health` exposing it — **done, 2026-09-25.** All six collectors report to an in-memory tracker (`app/feeds.py`): aircraft, AIS and satellites since 2026-09-24, and now the IODA connectivity poller, the RSS lane and the Telegram listener. The three tracking routes return the envelope; `GET /health` reads the trackers from memory; and a single flusher (`app/feed_store.py`) writes `feed_health` (current facts per feed), `feed_transition` (state changes, plus a `process_start` row per feed at boot) and `feed_heartbeat` (one row per feed per minute with attempts, successes and failures by kind) — decision 3 above. `GET /health/history?feed=` returns the liveness timeline with any heartbeat gap over 120 s as an explicit `unknown` segment; checked against a real 3-minute backend stop, which read as 185 s of `unknown` between two `process_start` rows. How the last three read: **connectivity**'s upstream clock is the newest real IODA measurement across every country and sensor, and a cycle with zero answers is a failure (before, it set a status string and the country list read as calm). **RSS** fetches every feed, reports, *then* classifies, so its health is whether the outlets answered and not how long the classifier took; one feed answering is a success (no partial state, decision 1), and a new `parts` field on the tracker names every feed's own outcome so a dead URL stays visible behind a live roll-up. Its clock is the newest article carrying a real `pubDate` — an undated item was stamped with receipt time and now says so (`dated: false`) instead of passing for the feed's time. **Telegram** is event-driven, so a watchdog reports the connection once a minute and the clock is the newest message seen: connected-but-quiet reads `stale`, a drop reads `retrying` then `unavailable`, and a listener that exited reads `dead` *with* its last error (a session that wants a login code is `auth_failed`, not a network error). In demo mode RSS and Telegram are not run and read `unconfigured` with the reason "not run in demo mode" rather than pending forever. Nothing in the frontend reads `/health` yet; that is Phase 3. The thresholds for the three new feeds are as unmeasured as the first three. The three tables come from `create_all` (whole new tables, which it can create), so they add nothing to the hand-kept ALTER list `C63` describes — but a column added to them later would.
...
| 2026-09-24 | `5a10ca2` | `feed_health` schema decisions written before any code (Roadmap, Phase 2): one worst-first `FeedState` vocabulary with `unconfigured` outside the roll-up and no `partial`/`degraded`/`down`; a closed `error_kind`; `fallback_from` and `synthetic` as fields; facts stored and state derived at read time from three clocks and the server's clock; transition plus per-minute heartbeat rows rather than per-attempt rows; one envelope with a nullable count on the three tracking routes. Thresholds marked unmeasured. Two decisions left open, neither blocking the aircraft slice |
| 2026-09-24 | `2ba3a2e` | The aircraft slice of `feed_health`. `app/feeds.py`: the `FeedState` vocabulary, a closed `ErrorKind`, the feed registry and a pure tracker holding three clocks, with state derived at read time. The aircraft poller records every attempt (fetchers now return what happened and why, not `list \| None`); `fallback_from` names the adsb.lol→OpenSky switch; demo mode records `synthetic`. `/tracking/aircraft` returns the envelope; `GET /health` reads the trackers from memory; `/tracking/jamming` withholds its verdict when the feed is not live. Frontend: header, rail, FEED CURRENCY and both map legends read the server's state, and print "—" with a reason instead of a number nobody can vouch for. Closes `C31` for aircraft; the strict xfail from `2b60071` passes and its marker is gone. Checked in a running app: live, a network cut mid-run (retrying at 53 s, unavailable at 5 min, recovery to live in one poll), a cold start with no network, and demo mode. adsb.lol's `now` confirmed as milliseconds against a live response. 199 passed; four defects re-introduced, four catches. Ledger recounted: it had drifted by one again |
| 2026-09-24 | `108d38e` | The AIS slice of `feed_health`. `maritime.py`: a missing key is recorded as `unconfigured` (named under `/health`'s `not_collected`) instead of returning silently; liveness follows accepted position reports, with the upstream clock taken from each message's `time_utc`; every read has a 300 s deadline, so an open-but-silent socket is recycled; reconnects climb a 5/15/60/300 s ladder then 900 s, and a key error frame waits an hour, replacing a fixed 10 s loop (closes the untriaged maritime reconnect item); `get_vessels` ages rows against the last success during an outage. `/tracking/vessels` returns the envelope; header, rail, FEED CURRENCY (which now names unconfigured feeds instead of calling them silent) and both legends read it. Checked in a running app with no key, with a bogus key against the real AISStream, and in demo mode. **The bogus key did not produce an error frame**: AISStream dropped the socket without one, so a bad key reads as a transport failure (recorded under the closed untriaged item). 215 passed; five defects re-introduced, five catches, the no-deadline one now failing in seconds instead of hanging the suite |
| 2026-09-24 | `dae58d9` | The satellites slice of `feed_health`. `satellites.py`: epochs parsed from line 1 and checksums verified on both lines, malformed records rejected and counted; `fetch_once` reports every outcome to the tracker (a 403/429 is `rate_limited`, a 200 with no valid record `parse_error`, both keeping the cache as before); the upstream clock is the newest epoch, so orbits older than 14 days read `stale` even on a fresh fetch; a disk-cache load is labelled with its real fetch time, and old cache files get their epochs re-derived. `/tracking/tle` returns the envelope; the header SAT count, a new SATELLITES rail row (newest element-set age beside fetch age), FEED CURRENCY and the globe legend read it, and the browser now polls the backend every 5 min instead of 6 h so a recovery shows in minutes. Closes the untriaged TLE-epoch item and, with the AIS slice, the bare-array tracking-routes item. Checked in a running app against a real pre-change disk cache (24 records, epochs re-derived, newest 13 h old, labelled 1.5 h old). 224 passed; five defects re-introduced, five catches |
| 2026-09-24 | `b40ca4e` | `feed_health` persistence (decision 3). Three tables — `feed_health`, `feed_transition`, `feed_heartbeat` — written by one flusher every 60 s from the trackers, never by the pollers; a `process_start` transition per feed at boot; per-minute attempt/success/failure-by-kind counters on the tracker, drained only after a successful commit, so a database error loses nothing and never reaches a poller; 30-day heartbeat retention. `GET /health/history?feed=&hours=` returns the liveness timeline, with any heartbeat gap over 120 s as `unknown` rather than the last state stretched across it. Checked on a running backend: 4 aircraft attempts a minute (the 15 s cadence, now measured rather than assumed), and a 3-minute stop read back as 185 s of `unknown` between two `process_start` rows. 234 passed; four defects re-introduced, four catches. The roadmap box stays unticked: connectivity, RSS and Telegram are not registered feeds yet |
| 2026-09-25 | `b9dab57` | `feed_health`: connectivity, RSS and Telegram registered — all six collectors now report, and the Phase 2 box is ticked. IODA's clock is its newest real measurement and zero answers is a failure; RSS fetches all feeds, reports, then classifies, with per-feed outcomes in a new `parts` field and undated items marked `dated: false` instead of carrying receipt time; Telegram gets a once-a-minute connection watchdog, the newest message as its clock, and login-code prompts classified `auth_failed`. `dead` now carries the last error the collector reported. Demo mode names RSS and Telegram as not run. Checked on a running backend: connectivity and RSS went live on their first cycles against the real upstreams, and RSS's `parts` exposed four dead feeds (untriaged, Collection). 261 passed; fourteen defects re-introduced, fourteen catches |
| 2026-09-25 | `0f6e1ea` | Security: `C60`, `C61`, `C62` fixed. `/events/admin/*` moved onto one router behind `X-Admin-Token` (`ADMIN_TOKEN`; unset answers 503), with a test that fails on any admin path or write route outside it; CORS narrowed from reflect-any-origin-with-credentials to a `CORS_ORIGINS` allow-list, `GET` only, no credentials; the websocket refuses foreign browser origins; compose publishes all three ports on 127.0.0.1, builds `DATABASE_URL` from `POSTGRES_*`, and stops handing the frontend container every backend secret. Checked live: the finding's attacker preflight now answers 400 with no `allow-origin`, admin calls 401/401/200 for none/wrong/right token, the attacker websocket gets 403 and the frontend's is accepted, and the real frontend still loads its events, aircraft and vessels. 284 passed; eight defects re-introduced, eight catches |
| 2026-09-25 | *this commit* | Close `C69`: demo bylines become `DEMO-CH-01` to `DEMO-CH-11`, every newly generated demo row starts its summary and raw_text with `SYNTHETIC · `, and nine templates stop putting words in real actors' mouths. Three tests in `tests/unit/test_demo.py`; four defects re-introduced, four catches; 287 passed after rebasing onto `0f6e1ea` (237 on the original base, `b40ca4e`). Narrows `C71` |
