...
- [ ] `feed_health` table written from each poller's existing loop; `/health` exposing it — **nearly done, 2026-09-24.** The aircraft, AIS and satellite pollers report every attempt to an in-memory tracker (`app/feeds.py`); all three tracking routes return the envelope; `GET /health` reads the trackers from memory; and a single flusher (`app/feed_store.py`) writes `feed_health` (current facts per feed), `feed_transition` (state changes, plus a `process_start` row per feed at boot) and `feed_heartbeat` (one row per feed per minute with attempts, successes and failures by kind) — decision 3 above. `GET /health/history?feed=` returns the liveness timeline with any heartbeat gap over 120 s as an explicit `unknown` segment; checked against a real 3-minute backend stop, which read as 185 s of `unknown` between two `process_start` rows. **What keeps the box unticked:** the connectivity poller, the RSS lane and the Telegram listener are not registered feeds yet, so "each poller" is three of six. The three new tables come from `create_all` (whole new tables, which it can create), so they add nothing to the hand-kept ALTER list `C63` describes — but a column added to them later would.
