...
- /tracking/aircraft, /tracking/vessels and /tracking/tle all return bare arrays with no as_of and no coverage (routes/tracking.py:11-39); maritime.py:134 maintains `_cache["last_update"]` that nothing ever reads. Header.tsx:127 renders "0 SAT" for a dead CelesTrak feed exactly as it would for an empty sky, and "0 VES" likewise - the status…
  **Aircraft half fixed 2026-09-24:** `/tracking/aircraft` now returns the feed envelope (FINDINGS decision 4); vessels and TLEs remain bare arrays, so "0 VES" is still a possible false zero.
...
- [ ] `feed_health` table written from each poller's existing loop; `/health` exposing it — **aircraft slice shipped 2026-09-24**: the in-memory tracker, the envelope on `/tracking/aircraft`, and `GET /health` reading the trackers. Not yet: the other feeds (AIS next, then TLE), and persistence (the transition and heartbeat rows of decision 3), which is what ticks this box
